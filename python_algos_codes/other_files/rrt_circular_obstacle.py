...
class RRT:
    ...
    def __init__(self, start, goal, obstacle_list, rand_area, expand_dis=3.0, goal_sample_rate=5, max_iter=500):
        ...
        self.obstacle_list = obstacle_list  # List of (x, y, radius)
    ...
    def steer(self, from_node, to_node, extend_length=float("inf")):
        ...
        # Cap the distance to expand_dis
    ...
    def get_random_node(self):
        """Samples a random point in space, occasionally sampling the goal to bias the search."""
    ...
    def get_nearest_node_index(self, node_list, rnd_node):
        """Finds the index of the closest node in the tree to the random node."""
        dlist = [(node.x - rnd_node.x) ** 2 + (node.y - rnd_node.y) ** 2 for node in node_list]
        minind = dlist.index(min(dlist))
        return minind

    def check_collision(self, from_node, to_node, obstacle_list):
        """Checks if the line segment between two nodes intersects any circular obstacles."""
        for (ox, oy, size) in obstacle_list:
            # Check distance from the new node to the obstacle
            dx = to_node.x - ox
            dy = to_node.y - oy
            d = math.hypot(dx, dy)
            if d <= size:
            ...
            # Basic check along the path (interpolate points)
            steps = 5
