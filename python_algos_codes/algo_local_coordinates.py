    ...
    def run_astar(self):
        ...
        while open_set:
            ...
            if current == self.goal_node:
                ...
                #     self.astar_path.append([round(current.x, 3), round(current.y, 3)])
                #     current = came_from[current]
                # self.astar_path.append([round(start_node.x, 3), round(start_node.y, 3)])
...
if __name__ == "__main__":
    ...
    if final_coordinate_list:
        print("path_array =", final_coordinate_list)
        print(f"\nSUCCESS! Found a path with {len(final_coordinate_list)} waypoints:")
        print(f"Total Distance: {planner.path_length:.2f} meters")
